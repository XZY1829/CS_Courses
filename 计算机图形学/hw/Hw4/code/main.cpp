...
// 你的实现没有什么大的问题，已经正确实现了de Casteljau算法的递归形式。
// 以下是建议和说明：
// 1. 唯一注意：cv::Mat用像素索引，在bezier函数中如果递归结果point的坐标出界，会导致越界访问，可以在bezier里检查point是否在图像范围内。
// 2. 若想提升效率，可考虑非递归实现。本递归写法正确可用。
// 3. t 建议用 double 以减少精度损失。
...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f> &control_points,
  ...
  for (size_t i = 0; i + 1 < control_points.size(); ++i) {
    new_control_points.emplace_back(control_points[i] * (1 - t) +
