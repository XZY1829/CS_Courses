...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) {
    ...
    const std::array<Eigen::Vector2f, 4> sample_offsets = {
    ...
    for (int x = min_x; x <= max_x; x++) {
        for (int y = min_y; y <= max_y; y++) {
            ...
            for (int s = 0; s < msaa_samples; ++s) {
                const float px = x + sample_offsets[s].x();
                const float py = y + sample_offsets[s].y();
                ...
                if (!std::isfinite(z_interpolated)) {
                    continue;
                }

                int sample_index = index * msaa_samples + s;
                if (z_interpolated >= msaa_depth_buf[sample_index]) {
                    continue;
                }

                msaa_depth_buf[sample_index] = z_interpolated;
                ...
                Vector3f interpolated_view_pos =
                ...
                interpolated_texcoords.x() = std::clamp(interpolated_texcoords.x(), 0.0f, kUvMax);
                interpolated_texcoords.y() = std::clamp(interpolated_texcoords.y(), 0.0f, kUvMax);
                if (!std::isfinite(interpolated_texcoords.x()) || !std::isfinite(interpolated_texcoords.y())) {
                    interpolated_texcoords = Vector2f(0.5f, 0.5f);
                }
                ...
                msaa_color_buf[sample_index] = fragment_shader(payload);
                sample_updated = true;
            }

            if (sample_updated) {
                Vector3f resolved_color = Vector3f::Zero();
                float min_depth = std::numeric_limits<float>::infinity();
                for (int s = 0; s < msaa_samples; ++s) {
                    int sample_index = index * msaa_samples + s;
                    resolved_color += msaa_color_buf[sample_index];
                    min_depth = std::min(min_depth, msaa_depth_buf[sample_index]);
                }
                frame_buf[index] = resolved_color / static_cast<float>(msaa_samples);
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h) {
    ...
    msaa_color_buf.resize(w * h * msaa_samples);
    msaa_depth_buf.resize(w * h * msaa_samples);
