...
void Renderer::Render(const Scene &scene) {
  ...
  for (int j = 0; j < scene.height; ++j) {
    for (int i = 0; i < scene.width; ++i) {
      // generate primary ray direction: map pixel center to the z = -1 plane
      float x = (2 * (i + 0.5f) / scene.width - 1.f) * imageAspectRatio * scale;
      float y = (1.f - 2 * (j + 0.5f) / scene.height) * scale;
