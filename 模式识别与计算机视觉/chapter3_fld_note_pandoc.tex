\section{第三章：特征归一化与 Fisher
线性判别分析（FLD）笔记}\label{ux7b2cux4e09ux7ae0ux7279ux5f81ux5f52ux4e00ux5316ux4e0e-fisher-ux7ebfux6027ux5224ux522bux5206ux6790fldux7b14ux8bb0}

\begin{quote}
课程：模式识别与计算机视觉（PRCV）\\
对应课件：\texttt{3\_Normalization\_FLD.pdf}\\
复习重点：\textbf{FLD
的目标函数、矩阵化、优化推导、多分类扩展、数值稳定性}
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{0. 本章主线}\label{ux672cux7ae0ux4e3bux7ebf}
...
\tightlist
\item
  \textbf{先把输入特征处理好（归一化）}，避免量纲和尺度干扰模型。\\
\item
  \textbf{再做监督判别特征提取（FLD）}，直接围绕``分类可分性''优化投影方向。
\end{enumerate}
...
\subsection{1. 特征归一化（页
14--21）}\label{ux7279ux5f81ux5f52ux4e00ux5316ux9875-1421}

\subsection{1.1 每维 Min-Max
归一化}\label{ux6bcfux7ef4-min-max-ux5f52ux4e00ux5316}
...
\subsection{\texorpdfstring{1.2 \(\ell_2/\ell_1\)
归一化（按样本向量）}{1.2 \textbackslash ell\_2/\textbackslash ell\_1 归一化（按样本向量）}}\label{ell_2ell_1-ux5f52ux4e00ux5316ux6309ux6837ux672cux5411ux91cf}
...
\subsection{1.3 Z-score 标准化}\label{z-score-ux6807ux51c6ux5316}
...
\subsection{1.4 Robust
Scaling（抗离群值）}\label{robust-scalingux6297ux79bbux7fa4ux503c}
...
\subsection{1.5
测试集归一化铁律（高频考点）}\label{ux6d4bux8bd5ux96c6ux5f52ux4e00ux5316ux94c1ux5f8bux9ad8ux9891ux8003ux70b9}

\textbf{只能用训练集参数归一化测试集}，不能在测试集重新估计
\(min/max\)、\(\mu,\sigma\)、median、IQR。\\
交叉验证同理：每一折都只用该折训练子集估参数。

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{2. 为什么需要 FLD（页
22--24）}\label{ux4e3aux4ec0ux4e48ux9700ux8981-fldux9875-2224}
...
\begin{quote}
找投影方向
\(\mathbf{w}\)，使\textbf{投影后类别中心尽量远}，同时\textbf{各类内部尽量紧凑}。
\end{quote}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{3. 二分类 FLD 的完整推导（重点，页
25--30）}\label{ux4e8cux5206ux7c7b-fld-ux7684ux5b8cux6574ux63a8ux5bfcux91cdux70b9ux9875-2530}

以下先讲二分类 \(y\in\{1,2\}\)。

\subsection{3.1 问题设定}\label{ux95eeux9898ux8bbeux5b9a}
...
\subsection{3.2 为什么不能只最大化
(m\_2-m\_1)\^{}2}\label{ux4e3aux4ec0ux4e48ux4e0dux80fdux53eaux6700ux5927ux5316-m_2-m_12}
...
则 \(\mathbf{w}\to\) \(\alpha\mathbf{w\)\} 时目标会随 \(\alpha^2\)
无界放大，且不考虑类内分散。\\
因此需要同时惩罚类内散布。

\subsection{3.3 Fisher 准则}\label{fisher-ux51c6ux5219}
...
总类内散度：s\_1\textsuperscript{2+s\_2}2。
...
\subsection{3.4 矩阵化推导}\label{ux77e9ux9635ux5316ux63a8ux5bfc}
...
\subsection{3.5
拉格朗日法求解}\label{ux62c9ux683cux6717ux65e5ux6cd5ux6c42ux89e3}
...
\subsection{3.6
二分类闭式解}\label{ux4e8cux5206ux7c7bux95edux5f0fux89e3}
...
\subsection{3.7
分类阈值（实战常用）}\label{ux5206ux7c7bux9608ux503cux5b9eux6218ux5e38ux7528}
...
\subsection{4. 二分类 FLD
算法步骤（可背）}\label{ux4e8cux5206ux7c7b-fld-ux7b97ux6cd5ux6b65ux9aa4ux53efux80cc}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  计算 \(\boldsymbol{\mu}_1,\boldsymbol{\mu}_2\)\\
\item
  计算 S\_W=S\_1+S\_2\\
\item
  计算
  \(\mathbf{w}^\star=S_W^{-1}(\boldsymbol{\mu}_2-\boldsymbol{\mu}_1\))\\
\item
  归一化 \(\mathbf{w}^\star\)\\
\item
  对样本做投影 \(u=\mathbf{w}^{\star\top}\mathbf{x\)\} 并设阈值分类
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{5. S\_W 不可逆时怎么办（页
31）}\label{s_w-ux4e0dux53efux9006ux65f6ux600eux4e48ux529eux9875-31}

常见于 n\textless d（小样本高维）：
...
\subsection{6. 多分类 FLD（页
32--33，重点）}\label{ux591aux5206ux7c7b-fldux9875-3233ux91cdux70b9}

\subsection{6.1
多类散度矩阵}\label{ux591aux7c7bux6563ux5ea6ux77e9ux9635}

设类别 \(i=1,\dots,C\)，样本数 N\_i，总样本数 \(N=\sum_i\) N\_i。
...
\subsection{6.2 优化问题}\label{ux4f18ux5316ux95eeux9898}
...
\subsection{6.3 为什么最多只有 C-1
个有效方向}\label{ux4e3aux4ec0ux4e48ux6700ux591aux53eaux6709-c-1-ux4e2aux6709ux6548ux65b9ux5411}
...
直观上：C 个类均值相对全局均值的偏移向量线性相关（和为0），独立维数最多
C-1。\\
所以 FLD 的判别子空间维度上限是 C-1。

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{7. PCA + FLD（Fisherfaces，页
40）}\label{pca-fldfisherfacesux9875-40}
...
\subsection{8. FLD 主要变体（页
34--39）}\label{fld-ux4e3bux8981ux53d8ux4f53ux9875-3439}
...
\subsection{9.
易错点与考试速记}\label{ux6613ux9519ux70b9ux4e0eux8003ux8bd5ux901fux8bb0}

\subsection{9.1 易错点}\label{ux6613ux9519ux70b9}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  把 PCA 当分类器：错，PCA 不用标签。\\
\item
  忘记分母类内散度：只拉开均值不够。\\
\item
  测试集单独归一化：数据泄漏。\\
\item
  S\_W 奇异时硬求逆：应使用伪逆/正则/PCA 预降维。\\
\item
  多类 FLD 维度上限写错：应是 C-1，不是 C。
\end{enumerate}

\subsection{9.2 公式速查}\label{ux516cux5f0fux901fux67e5}
...
本章的核心能力是：\textbf{会写出 Fisher
准则、会从散度定义推到矩阵形式、会从拉格朗日推到广义特征值问题、知道多类上限为何是
C-1}。
