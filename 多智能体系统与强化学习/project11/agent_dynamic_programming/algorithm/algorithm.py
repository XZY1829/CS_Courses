...
class Algorithm:
    ...
    def __init__(self, gamma, theta, episodes, state_size, action_size, logger):
        ...
        self.treasure_bonus = 0.9
        self.progress_bonus = 0.25
        self.retreat_penalty = 0.8
        self.treasure_leave_penalty = 1.2
        self.treasure_states = set()
        self.goal_state = None

    def set_treasure_ids(self, treasure_ids):
        self.treasure_states.clear()
        for tid in treasure_ids:
            if tid in self.TREASURE_COORDS:
                x, y = self.TREASURE_COORDS[tid]
                self.treasure_states.add(int(x * 64 + y))

    def set_goal(self, end_xy):
        if not end_xy or len(end_xy) != 2:
            self.goal_state = None
            return
        self.goal_state = int(end_xy[0] * 64 + end_xy[1])

    def _manhattan_to_goal(self, state):
        if self.goal_state is None:
            return None
        sx, sy = divmod(int(state), 64)
        gx, gy = divmod(int(self.goal_state), 64)
    ...
    def policy_iteration(self, F):
        ...
        while i < self.episodes:
            ...
            new_policy = self.policy_improvement(Q, F)
    ...
    def value_iteration(self, F):
        ...
        while i < self.episodes:
            ...
            policy = self.policy_improvement(self.q_value_iteration(V, F), F)
    ...
    def policy_improvement(self, Q, F=None):
        ...
        for state in range(self.state_size):
            ...
            best_action = int(best_actions[0])

            # Tie-break by choosing the successor closest to the goal.
            # 如果价值相同，优先选择更靠近终点的后继动作
            if F is not None and len(best_actions) > 1 and self.goal_state is not None:
                best_distance = float("inf")
                for action in best_actions:
                    try:
                        next_state, _, _ = F[str(state)][str(int(action))]
                        next_distance = self._manhattan_to_goal(next_state)
                        if next_distance is not None and next_distance < best_distance:
    ...
    def _get_value(self, state, action, F, V):
        ...
        try:
            next_state, reward, done = F[str(state)][str(action)]
            if reward == 0:
                reward = self.step_penalty

            # Progress shaping towards goal to avoid local loops.
            # 终点势能引导，抑制局部循环
            cur_d = self._manhattan_to_goal(state)
            nxt_d = self._manhattan_to_goal(next_state)
            progress = 0
            if cur_d is not None and nxt_d is not None:
                progress = cur_d - nxt_d
                if progress > 0:
                    reward += self.progress_bonus * progress
                elif progress < 0:
                    reward += self.retreat_penalty * progress

            # Treasure bonus is only granted when simultaneously moving toward goal.
            # 宝箱奖励仅在同时朝终点前进时给出，防止拿到后原地绕圈
            if next_state in self.treasure_states and next_state != state and progress > 0:
                reward += self.treasure_bonus

            # Strongly discourage leaving a treasure cell without making goal progress.
            # 强烈惩罚从宝箱离开却没有继续朝终点推进
            if state in self.treasure_states and next_state != state and progress <= 0:
                reward -= self.treasure_leave_penalty
