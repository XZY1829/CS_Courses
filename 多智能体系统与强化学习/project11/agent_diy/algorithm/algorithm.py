...
class Algorithm:
    def __init__(self, gamma, learning_rate, state_size, action_size):
        ...
        self.visit_count = np.zeros([self.state_size, self.action_size], dtype=np.int32)

    def set_episode(self, episode, max_episode):
        # Slow decay on alpha to improve late-stage stability.
        # 缓慢衰减学习率，提升后期稳定性
        decay_ratio = min(1.0, episode / max(1, max_episode))
        self.alpha = max(0.05, self.learning_rate * (1.0 - 0.7 * decay_ratio))
    ...
    def learn(self, list_sample_data):
        ...
        delta = reward + self.gamma * best_next_q - self.Q[state, action]

        self.visit_count[state, action] += 1
        adaptive_alpha = self.alpha / (1.0 + 0.002 * self.visit_count[state, action])
        self.Q[state, action] += adaptive_alpha * delta
