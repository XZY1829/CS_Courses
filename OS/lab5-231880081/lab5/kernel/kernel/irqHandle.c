...
void syscallOpen(struct StackFrame *sf) {
    ...
    if (ret == 0) { // file exist
        ...
        if ((sf->edx >> 3) % 2 == 0) {
            ...
            tmp = REGULAR_TYPE;
            ret = allocInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset,
                             str + size + 1, tmp);
...
void syscallWriteFile(struct StackFrame *sf) {
    ...
    while (left_num > 0) {
        if (cur >= inode.blockCount) {
            ...
            if (ret < 0) {
                inode.size = file[sf->ecx - MAX_DEV_NUM].offset + size - left_num > inode.size ?
                             file[sf->ecx - MAX_DEV_NUM].offset + size - left_num : inode.size;
...
void syscallRemove(struct StackFrame *sf) {
    ...
    if (ret == 0) { // file exist
        ...
        if (destInode.type == REGULAR_TYPE) {
            ...
            ret = freeInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset,
                            str + size + 1, REGULAR_TYPE);
